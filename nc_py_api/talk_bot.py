...
class TalkBot:
    ...
    async def enabled_handler(self, enabled: bool, nc: NextcloudApp) -> None:
    ...
    async def _sign_send_request(
        ...
        secret = await get_bot_secret(self.callback_url)
        if secret is None:
...
# Backward compatibility alias (will be removed in v1.0.0)
...
async def get_bot_secret(callback_url: str) -> bytes | None:
    ...
    secret_value = await NextcloudApp().appconfig_ex.get_value(secret_key)
