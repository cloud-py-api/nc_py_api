...
class UsersGroupsAPI:
    ...
    def create(self, group_id: str, display_name: Optional[str] = None) -> dict:
        params = {"groupid": group_id}
        if display_name is not None:
            params["displayname"] = display_name
        return self._session.ocs(method="POST", path=f"{ENDPOINT}", params=params)

    def edit(self, group_id: str, display_name: str) -> dict:
        params = {"key": "displayname", "value": display_name}
        return self._session.ocs(method="PUT", path=f"{ENDPOINT}/{group_id}", params=params)
