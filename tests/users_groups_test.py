...
def test_group_get_list(nc):
    ...
    groups = nc.users.groups.get_list(mask=TEST_GROUP_NAME)
    assert len(groups) == 1
    groups = nc.users.groups.get_list(limit=1)
    assert len(groups) == 1
    assert groups[0] != nc.users.groups.get_list(limit=1, offset=1)[0]
    nc.users.groups.delete(TEST_GROUP_NAME)
    nc.users.groups.delete(TEST_GROUP_NAME2)


@pytest.mark.skipif(not isinstance(NC_TO_TEST[:1][0], Nextcloud), reason="Not available for NextcloudApp.")
@pytest.mark.parametrize("nc", NC_TO_TEST[:1])
def test_get_non_existing_group(nc):
    groups = nc.users.groups.get_list(mask="Such group should not be present")
    assert isinstance(groups, list)
    assert not groups
...
def test_app_mode():
    groups_list = NC_APP.users.groups.get_list()
