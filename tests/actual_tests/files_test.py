...
@pytest.mark.asyncio(scope="session")
async def test_find_files_listdir_depth_async(anc_any):
    result = await anc_any.files.find(["and", "gt", "size", 0, "like", "mime", "image/%"], path="test_dir")
    assert len(result) == 2
    result2 = await anc_any.files.find(["and", "gt", "size", 0, "like", "mime", "image/%"], path="/test_dir")
    assert len(result2) == 2
    assert result == result2
    result = await anc_any.files.find(["and", "gt", "size", 0, "like", "mime", "image/%"], path="test_dir/subdir/")
    assert len(result) == 1
    result = await anc_any.files.find(["and", "gt", "size", 0, "like", "mime", "image/%"], path="test_dir/subdir")
    assert len(result) == 1
    result = await anc_any.files.find(["and", "gt", "size", 1024 * 1024, "like", "mime", "image/%"], path="test_dir")
    assert len(result) == 0
    result = await anc_any.files.find(
        ["or", "and", "gt", "size", 0, "like", "mime", "image/%", "like", "mime", "text/%"], path="test_dir"
    )
    assert len(result) == 6
    result = await anc_any.files.find(["eq", "name", "test_12345_text.txt"], path="test_dir")
    assert len(result) == 2
    result = await anc_any.files.find(["like", "name", "test_%"], path="/test_dir")
    assert len(result) == 9
    assert not await anc_any.files.find(["eq", "name", "no such file"], path="test_dir")
    assert not await anc_any.files.find(["like", "name", "no%such%file"], path="test_dir")
    result = await anc_any.files.find(["like", "mime", "text/%"], path="test_dir")
    assert len(result) == 4


@pytest.mark.asyncio(scope="session")
async def test_find_files_datetime_async(anc_any):
    time_in_past = datetime.now() - timedelta(days=1)
    time_in_future = datetime.now() + timedelta(days=1)
    assert len(await anc_any.files.find(["gt", "last_modified", time_in_past], "/test_dir/subdir/"))
    assert not len(await anc_any.files.find(["gt", "last_modified", time_in_future], "/test_dir/subdir/"))
    assert not len(await anc_any.files.find(["lt", "last_modified", time_in_past], "/test_dir/subdir/"))
    assert len(await anc_any.files.find(["lt", "last_modified", time_in_future], "/test_dir/subdir/"))
...
@pytest.mark.asyncio(scope="session")
async def test_fs_node_fields_async(anc_any):
    results = await anc_any.files.listdir("/test_dir")
    ...
    for _, result in enumerate(results):
        ...
        res_by_id = await anc_any.files.by_id(result.file_id)
        assert res_by_id
        res_by_path = await anc_any.files.by_path(result.user_path)
        assert res_by_path
...
@pytest.mark.asyncio(scope="session")
async def test_fs_node_str_async(anc_any):
    fs_node1 = await anc_any.files.by_path("test_empty_dir_in_dir")
    ...
    fs_node2 = await anc_any.files.by_path("test_12345_text.txt")
...
@pytest.mark.asyncio(scope="session")
async def test_fs_node_is_xx_async(anc_any):
    folder = (await anc_any.files.listdir("test_empty_dir", exclude_self=False))[0]
